...
func apilist() {
	for _, group := range apiConfig {
		...
		for _, api := range group.Apis {
			...
			godocLink := fmt.Sprintf("https://pkg.go.dev/github.com/fastwego/dingding/apis/%s?tab=doc#%s", group.Package, api.FuncName)
...
func build(group ApiGroup) {
	...
	for _, api := range group.Apis {
		...
		isUploadApi := false
		if group.Package == "cspace" && (api.FuncName == "Upload" || api.FuncName == "UploadSingle" || api.FuncName == "UploadChunk") {
		...
		switch {
		case strings.Contains(api.Request, "GET http"):
			_TEST_ARGS_STRUCT_ = `ctx *dingding.App, ` + _GET_PARAMS_
		case strings.Contains(api.Request, "POST http"):
			_TEST_ARGS_STRUCT_ = `ctx *dingding.App, payload []byte`
			if _GET_PARAMS_ != "" {
...
func _FUNC_NAME_(ctx *dingding.App, payload []byte_GET_PARAMS_) (resp []byte, err error) {
...
func _FUNC_NAME_(ctx *dingding.App_GET_PARAMS_) (resp []byte, err error) {
