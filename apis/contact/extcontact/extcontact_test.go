...
func TestListLabelGroups(t *testing.T) {
	...
	var resp []byte
	test.MockSvrHandler.HandleFunc(apiListLabelGroups, func(w http.ResponseWriter, r *http.Request) {
		w.Write([]byte(resp))
	})
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			gotResp, err := ListLabelGroups(tt.args.ctx, tt.args.payload)
			//fmt.Println(string(gotResp), err)
			if (err != nil) != tt.wantErr {
				t.Errorf("ListLabelGroups() error = %v, wantErr %v", err, tt.wantErr)
				return
			}
			if !reflect.DeepEqual(gotResp, tt.wantResp) {
				t.Errorf("ListLabelGroups() gotResp = %v, want %v", gotResp, tt.wantResp)
			}
		})
	}
}
