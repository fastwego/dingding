...
const (
	apiTranslate = "/topapi/ai/mt/translate"
	apiRecognize = "/topapi/ocr/structured/recognize"
)
...
func Translate(ctx *dingding.App, payload []byte) (resp []byte, err error) {
	return ctx.Client.HTTPPost(apiTranslate, bytes.NewReader(payload), "application/json;charset=utf-8")
}
...
func Recognize(ctx *dingding.App, payload []byte) (resp []byte, err error) {
	return ctx.Client.HTTPPost(apiRecognize, bytes.NewReader(payload), "application/json;charset=utf-8")
}
